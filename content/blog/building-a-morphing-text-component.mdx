...
Text changes on screen constantly. A counter ticks up. A price updates. A button label moves from `Run Simulation` to `Running Simulation` to `Simulation Done!`. A word shifts from `Laminar` to `Linear` to `Lamina`.
...
A morphing text component keeps that path visible. Individual characters enter, exit, and shift position as the string changes. The container width follows.

[demo: overview montage showing Laminar to Linear, Run Simulation to Running Simulation, and the number demo]
...
The correct unit for animation is a grapheme cluster, the thing a person perceives as a single character. `Intl.Segmenter` produces them.
...
For text, the right way to find surviving glyphs is the Longest Common Subsequence algorithm. LCS finds the largest set of units, in order, that appears in both the previous string and the next string. Those matched units keep their keys. Everything else is new.

[demo: text identity showing Laminar to Linear to Lamina. Show the main morph above a now/next key map where solid blue glyphs keep their keys and dashed gray glyphs swap.]
...
[demo: number identity showing the value increment and decrement. Show the main number above fixed dashed lanes, with the dollar sign in a gray dashed prefix cell and digit lanes in blue.]
...
[demo: animation layer showing one digit changing. Show the main digit above a two-row space/digit diagram: the blue dashed space stays fixed while the visible digit moves.]
...
The library ships four presets: `default`, `smooth`, `snappy`, and `bouncy`. Each produces a different character without changing any of the identity or layout logic.
...
`Run Simulation` and `Running Simulation` do not occupy the same space. A container that snaps to the new width instantly undermines the glyph animation, because the frame jumps before the characters have moved. A container that stays at the old width clips the new string partway through the transition.
...
[demo: auto-sizing showing Run Simulation to Running Simulation to Simulation Done! inside a blue dashed border. Below it, show autoSize off with a gray dashed border so the width change jumps instead of animating.]
...
The component is a chain of problems that each need to be solved in order. Grapheme splitting before identity. Identity before animation. Layout measurement before width animation. Motion configuration before the presets that sit on top of it. Each layer has one job and exposes a clean boundary to the layer above.

The result is a component that handles counters, prices, scores, labels, and button states without configuration. Pass it a value and it handles the rest.
